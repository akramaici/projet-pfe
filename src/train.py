    ...
    for epoch in range(epochs):
        ...
        with torch.no_grad():
            for images, targets in tqdm(val_loader, desc="Val"):
                images=images.to(device);  targets=[t.to(device) for t in targets]
                loss = criterion(model(images), targets)
                if not (torch.isnan(loss) or torch.isinf(loss)): val_loss += loss.item()
...
def plot_history(history, save_path):
    ...
    plt.axvline(best_ep, linestyle='--', label=f'Best epoch {best_ep} ({best_v:.4f})')
...
def run_training(dataset_path, checkpoint=None):
    ...
    nc = cfg['nc'];  print(f"Classes ({nc}) : {cfg['names']}")

    train_ds = YOLODataset(dataset_path/"train"/"images", dataset_path/"train"/"labels", IMG_SIZE, True)
    val_ds   = YOLODataset(dataset_path/"valid"/"images", dataset_path/"valid"/"labels", IMG_SIZE, False)

    train_ld = DataLoader(train_ds, BATCH_SIZE, shuffle=True,  num_workers=NUM_WORKERS,
                          pin_memory=(device=='cuda'), collate_fn=yolo_collate_fn)
    val_ld   = DataLoader(val_ds,   BATCH_SIZE, shuffle=False, num_workers=NUM_WORKERS,
                          pin_memory=(device=='cuda'), collate_fn=yolo_collate_fn)

    model = CustomYOLO(num_classes=nc, base_channels=BASE_CHANNELS, pretrained=True)
    ...
    model, h1 = train_one_phase(model, train_ld, val_ld, PHASE1_EPOCHS, PHASE1_LR,
                                device, IMG_SIZE, "checkpoints/best_phase1.pth", PHASE1_PATIENCE)

    # Phase 2
    print(f"\n{'='*60}\n  PHASE 2 : Fine-tuning complet\n{'='*60}")
    model.unfreeze_backbone()
    model, h2 = train_one_phase(model, train_ld, val_ld, PHASE2_EPOCHS, PHASE2_LR,
                                device, IMG_SIZE, "checkpoints/best_phase2.pth", PHASE2_PATIENCE)

    history = {'train_loss': h1['train_loss']+h2['train_loss'],
               'val_loss'  : h1['val_loss']  +h2['val_loss']}

    print(f"\nTotal : {len(history['train_loss'])} epochs")
    plot_history(history, "outputs/training_curves.png")
    return model, history, val_ds, val_ld, cfg['names'], nc, device
