...
def get_name(class_names, cls_id):
    if isinstance(class_names, dict): return class_names.get(cls_id, str(cls_id))
...
def predict_image(model, image_path, device, class_names, num_classes,
    ...
    if img is None: raise FileNotFoundError(f"❌ Image introuvable : {image_path}")
    img_rgb=cv2.cvtColor(img,cv2.COLOR_BGR2RGB);  H0,W0=img_rgb.shape[:2]
    print(f"📷 Image : {W0}x{H0}")
    img_lb,scale,pad_x,pad_y=letterbox(img_rgb,img_size)
    ...
    dets=decode_predictions(raw,conf_thresh=conf_thresh,num_classes=num_classes,
                            nms_thresh=nms_thresh,img_size=img_size)[0]
    print(f"🎯 {len(dets)} détections")
    fig,ax=plt.subplots(1,1,figsize=(14,6));  ax.imshow(img_rgb)
    for det in dets:
        px,py,pw,ph,conf,cls_id=det;  cls_id=int(cls_id)
        x1=int(((px-pw/2)*img_size-pad_x)/scale);  y1=int(((py-ph/2)*img_size-pad_y)/scale)
        x2=int(((px+pw/2)*img_size-pad_x)/scale);  y2=int(((py+ph/2)*img_size-pad_y)/scale)
...
def predict_video(model, video_path, device, class_names, num_classes,
    ...
    if not cap.isOpened(): raise FileNotFoundError(f"❌ Vidéo introuvable : {video_path}")
    ...
    while True:
        ...
        if frame_idx%50==0: print(f"  {frame_idx}/{TOTAL} frames...")
